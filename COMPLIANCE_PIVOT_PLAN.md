...
**Phase 2 progress:** Ask screen ✅ · real RBI Master Direction PDF ✅ (seeded) · **clause-level citation verification ✅** (§4.3/§7 — the judge's evidence quote is now grounded to a specific policy *clause* via a graded containment score, not a whole-chunk substring test; the verbatim clause + a "Verified" badge show in the gap row, and an `evidence_faithfulness` metric is gated in CI) · **change-tracking ✅** (a re-seeded circular keeps its `regulation_id`; the prior check snapshots the old requirements+verdicts, so `diff_requirements` — pure difflib, text-similarity, no LLM — classifies added/changed/unchanged/removed and `POST /api/compliance/recheck` re-judges only the deltas while carrying the rest forward, turning a ~34-call/~15-min full re-run into a few calls; UI: a re-check button per past check, a delta banner, "New"/"Changed" row chips) · **Library screen ✅** (3rd screen: "Your policies" upload + per-doc delete — `DELETE /api/documents/{filename}`, closing the Streamlit doc-delete parity gap — and a "Regulations" browse zone). **Phase 2 is complete** (all §9 items done). **Also done (post-Phase-2 quality):** clause/section-aware chunking for regulations ✅ (`split_legal_text` splits on clause/section markers and packs whole clauses instead of fixed token windows; opt-in via `clause_aware=`/`USE_CLAUSE_AWARE_CHUNKING`, enabled in the seed path — sharper requirement extraction + retrieval on dense legal text; quality gain measured by a keyed re-seed + `run_eval`). **Follow-ups (polish, optional):** retire Streamlit once at full parity (Next app still lacks the page-image view), re-seed the real RBI reg complete (with the `max_tokens` fix + clause-aware chunking), "Page 2.0"→"p.2" in chat citations. **Phase 3 (deployment) stays deferred** until quality/latency are demo-ready.
